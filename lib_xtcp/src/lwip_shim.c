...
static xtcp_error_code_t shim_ip_getsockopt(unsigned client_num, int32_t id, uint32_t option, uint8_t value[], uint32_t *length) {
  const struct ip_pcb *ip_pcb;

  switch (get_protocol(id)) {
...
xtcp_error_code_t shim_getsockopt(unsigned client_num, int32_t id, xtcp_socket_level_t level, uint32_t option, uint8_t value[], uint32_t *length) {
  ...
  case XTCP_SOCKET_LEVEL_IP:
    result = shim_ip_getsockopt(client_num, id, option, value, length);
    break;
  default:
    result = XTCP_EINVAL;
    break;
...
static xtcp_error_code_t shim_ip_setsockopt(unsigned client_num, int32_t id, uint32_t option, const uint8_t value[], uint32_t length) {
...
xtcp_error_code_t shim_setsockopt(unsigned client_num, int32_t id, xtcp_socket_level_t level, uint32_t option, const uint8_t value[], uint32_t length) {
  ...
  switch (level) {
  case XTCP_SOCKET_LEVEL_IP:
    return shim_ip_setsockopt(client_num, id, option, value, length);
  default:
    break;
  }
